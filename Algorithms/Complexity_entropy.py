...
def shannon_entropy(time_series,normalize=False):
    ...
    if normalize:
        ...
        ent=np.true_divide(ent, np.log2(len(freq_list)))
...
def ordinal_pat_prob(time_series, order=3, delay=1):
    """
    Examples
    --------
    1. Permutation entropy with order 2
    ...
    if v!=1:
        ...
        for i in range(1,len(x)):
            auxh2 = np.power(order, np.arange(order))
            hashmult=np.concatenate((hashmult, auxh2))

        #Embed x and sort the order of permutations

        sorted_idx=_embed(x[0], order=order, delay=delay).argsort(kind='quicksort')
        for i in range(1,len(x)):
            aux2 = _embed(x[i], order=order, delay=delay).argsort(kind='quicksort')
            sorted_idx=np.concatenate((sorted_idx, aux2), axis=1)
        # Associate unique integer to each permutations
        hashval = (np.multiply(sorted_idx, hashmult)).sum(1)
        # Return the counts
        _, c = np.unique(hashval, return_counts=True)
        # Use np.true_divide for Python 2 compatibility
        zeros=np.zeros(N-(len(c)))
        c1=np.concatenate((c, zeros), axis=None)
        p = np.true_divide(c1, c1.sum())
    else:
        N=factorial(order)
        #x = np.array(time_series)
        hashmult = np.power(order, np.arange(order))
...
def shannon_ent(probability,normalize=False):
    ...
    if normalize:
        #pe=np.true_divide(pe, pe_unif)
        pe /= np.log2(N)
        
    return pe

def equi_prob(order=3,dim=1):
    
    N=factorial(order)**dim
    ones=np.ones(N)
    #equilibrium uniform
    p_unif = np.true_divide(ones, ones.sum())
    
    #pe_unif = -np.multiply(p_unif, np.log2(p_unif)).sum()
    return p_unif

def MPR_entropy(time_series, order=3, delay=1):
    ...
    q_0=-2.0/(((N+1)/N)*np.log(N+1)-2*np.log(2*N)+np.log(N))
    
    P=ordinal_pat_prob(x, order, delay)
    P_e=equi_prob(order,dim)
    H_P=shannon_ent(P,normalize=True)
    
    q_j=(shannon_ent(np.true_divide((P+P_e),2),normalize=False) - (shannon_ent(P,normalize=False))/2
        -(shannon_ent(P_e,normalize=False))/2)*q_0
...
# x=[ 4,  7,  9, 10,  6, 11,  3]
# x_=[ [4,  7,  9, 10,  6, 11,  3],[15,  7,  9, 10,  6, 11,  3]]
# order=3
# delay=1
...
# probability=ordinal_pat_prob(x, order=3, delay=1)
# probability
...
# shannon_ent(probability,normalize=True)


# # In[217]:


# MPR_entropy(x, order=3, delay=1)
